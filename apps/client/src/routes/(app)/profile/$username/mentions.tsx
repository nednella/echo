...
export const Route = createFileRoute("/(app)/profile/$username/mentions")({
    loader: async ({ context: { queryClient }, params: { username } }) => {
        ...
        await queryClient.ensureInfiniteQueryData(profileFeedInfiniteQueryOptions(profile.id, "mentions"))
    },
    component: ProfileMentionsTab
})

function ProfileMentionsTab() {
    const { username } = Route.useParams()
    const { data: profile } = useSuspenseQuery(profileQueryOptions(username))

    return (
        <PostList
            options={profileFeedInfiniteQueryOptions(profile.id, "mentions")}
